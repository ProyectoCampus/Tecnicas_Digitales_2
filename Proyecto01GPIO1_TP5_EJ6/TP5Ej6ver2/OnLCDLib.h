...
OnLCDLib - LCD Library written in C for AVR Microcontrollers v1.5
...
#define LCD_DATA_START_PIN	4		// In 8-bit mode pins 0-7 will be used, in 4-bit mode 0-3 if 0 is first
#define LCD_RS_CONTROL_DDR 	DDRC 	//									 |
#define LCD_RW_CONTROL_DDR 	DDRC 	//									 |
#define LCD_E_CONTROL_DDR 	DDRC 	//									 |
#define LCD_RS_CONTROL_PORT PORTC 	// Port where RS, RW, E pins are	 |
#define LCD_RW_CONTROL_PORT PORTD 	// Port where RS, RW, E pins are	 |
#define LCD_E_CONTROL_PORT 	PORTB 	// Port where RS, RW, E pins are	 |
#define LCD_RS_PIN			PC0 	// Register selection signal		 |
#define LCD_RW_PIN			PC1 	// Read/write signal 				 |
#define LCD_E_PIN 			PC2 	// Enable signal					 |
//																		 |
// LCD type																 |
...
#if defined(DISPLAY_FLOATS) && DISPLAY_FLOATS == 1
void LCDWriteFloat(float float_number, int8_t nrOfDigits);
#endif
...
#define LCDWriteFloatXY(x, y, float_number, nrOfDigits)\
	LCDGotoXY(x, y);\
	LCDWriteFloat(float_number, nrOfDigits)\
...
void LCDWriteFloat(float float_number, int8_t nrOfDigits){
	...
	int16_t integer_part_2 = fractional_part * 100;
   
	// Convert fractional part to a positive 
	if(integer_part_2 < 0) integer_part_2 = -integer_part_2;
	if(float_number < 0 && integer_part_1 == 0) LCDWriteString("-");

	LCDWriteInt(integer_part_1, nrOfDigits);
	LCDWriteString(".");
	LCDWriteInt(integer_part_2, 0);
}
#endif
